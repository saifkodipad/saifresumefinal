...
const ChatBotWindow: React.FC<ChatBotWindowProps> = ({ onClose, isOpen }) => {
  ...
  return (
    <>
      {/* Overlay for mobile to capture clicks outside and provide better UX */}
      <div 
      ...
      <div 
        ...
        style={{ 
          ...
          maxHeight: isMinimized ? '3.5rem' : '85vh'
        }}
      ...
      >
        ...
        <div className="flex items-center justify-between p-3 bg-gradient-to-r from-pink-500 to-pink-600 text-white rounded-t-lg">
          <div className="flex items-center gap-2">
            <Bot size={20} />
            <h3 className="font-semibold text-sm md:text-base">Saif's Assistant</h3>
            <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
          </div>
          <div className="flex items-center gap-1">
            <button
              onClick={() => setIsMinimized(!isMinimized)}
              className="p-1.5 hover:bg-pink-500 rounded transition touch-manipulation"
        ...
        {!isMinimized && (
          <>
            {/* Messages */}
            <div className="flex-1 overflow-y-auto p-3 md:p-4 bg-pink-50/30">
            ...
            <div className="p-3 border-t border-pink-200/50 bg-white/50 backdrop-blur-sm rounded-b-lg">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 md:py-2 bg-white/90 border border-pink-200 rounded-full focus:outline-none focus:ring-2 focus:ring-pink-400 text-sm"
                  disabled={isLoading}
                />
                <button
                  onClick={handleSendMessage}
                  disabled={isLoading || !inputMessage.trim()}
                  className="px-4 py-2.5 md:py-2 bg-gradient-to-r from-pink-500 to-pink-600 text-white rounded-full hover:from-pink-600 hover:to-pink-700 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg touch-manipulation"
              ...
              <div className="flex gap-2 mt-3 overflow-x-auto pb-1 scrollbar-hide">
                <button
                  ...
                  className="text-xs bg-white/80 border border-pink-200 rounded-full px-3 py-1.5 text-pink-600 hover:bg-pink-100 whitespace-nowrap touch-manipulation"
                >
                  Projects
                </button>
                <button
